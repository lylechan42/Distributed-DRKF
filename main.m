...
diffusion_method = 'CI';   
estimation_method = 'DKF';

MSE_DKF = run_monte_carlo_simulation(...
...
num_runs = 1000;  % Number of Monte Carlo runs
method = 'moment_based';
parameter = 0.1;    % place holder

diffusion_enabled = true;  
diffusion_method = 'average';   
estimation_method = 'DKF';

MSE_DKF_average = run_monte_carlo_simulation(...
    sys, node_config, adj_matrix, x0, P0, T, vars, ...
    num_runs, method, parameter, diffusion_enabled, diffusion_method, estimation_method...
);
...
tic;
MSE_DKF_CI = run_monte_carlo_simulation(...
    sys, node_config, adj_matrix, x0, P0, T, vars, ...
    num_runs, method, parameter, diffusion_enabled, diffusion_method, estimation_method...
);

%%
plot_MSEs_new(MSE_DKF, [], 'Method', 'DKF');

plot_MSEs_new(MSE_DKF_average, [], 'Method', 'DKF_average');

plot_MSEs_new(MSE_DKF_CI, [], 'Method', 'DKF_CI');

%%
result_DKF = mean(MSE_DKF.avg_mse);
result_DKF_average = mean(MSE_DKF_average.avg_mse);
result_DKF_CI = mean(MSE_DKF_CI.avg_mse);
fprintf('\n=== Final Average MSE Results ===\n');
fprintf('%-20s: %.4f\n', 'DKF (No Diffusion)', result_DKF);
fprintf('%-20s: %.4f\n', 'DKF with Average', result_DKF_average);
fprintf('%-20s: %.4f\n', 'DKF with CI', result_DKF_CI);

%% KL_no
num_param = 2;
all_c = linspace(2,3.5,num_param);
num_runs = 1000;  % Number of Monte Carlo runs
...
%% W_no
num_param = 5;
all_rho = linspace(0.8,2,num_param);
num_runs = 1000;  % Number of Monte Carlo runs
method = 'wasserstein';
...
%% KL_no
plot_MSEs_new(MSE_DRKF_KL_no, all_c, ...
    'Method', 'Distributed KL Divergence', ...
    'ParamName', 'c', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% KL_average
plot_MSEs_new(MSE_DRKF_KL_average, all_c, ...
    'Method', 'Distributed KL Divergence', ...
    'ParamName', 'c', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% KL_CI
plot_MSEs_new(MSE_DRKF_KL_CI, all_c, ...
    'Method', 'Distributed KL Divergence', ...
    'ParamName', 'c', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% W_no
plot_MSEs_new(MSE_DRKF_W_no, all_rho, ...
    'Method', 'Distributed Wasserstein', ...
    'ParamName', 'c', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% W_average
plot_MSEs_new(MSE_DRKF_W_average, all_rho, ...
    'Method', 'Distributed Wasserstein', ...
    'ParamName', 'c', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% W_CI
plot_MSEs_new(MSE_DRKF_W_CI, all_rho, ...
    'Method', 'Distributed Wasserstein', ...
    'ParamName', 'c', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% M_no
plot_MSEs_new(MSE_DRKF_M_no, all_gamma, ...
    'Method', 'Distributed Moment Based', ...
    'ParamName', 'gamma', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% M_average
plot_MSEs_new(MSE_DRKF_M_average, all_gamma, ...
    'Method', 'Distributed Moment Based', ...
    'ParamName', 'gamma', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
%% M_CI
plot_MSEs_new(MSE_DRKF_M_CI, all_gamma, ...
    'Method', 'Distributed Moment Based', ...
    'ParamName', 'gamma', ...
    'PlotOptimal', true, ...
    'FigureSize', [1400 800]);
