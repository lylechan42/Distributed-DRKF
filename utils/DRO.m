function [S_optimal, U_optimal, Z_optimal, status, objective_value] = DRO(method, parameter, n, m, Sigma, quiet)
    ...
    if strcmp(method, 'moment_based')
        S_optimal = (1 + gamma) * Sigma;
        U_optimal = S_optimal(1:n, 1:n) - S_optimal(1:n, n+1:end) * ...
                    (S_optimal(n+1:end, n+1:end) \ S_optimal(n+1:end, 1:n));
        Z_optimal = NaN; % Z is not applicable in this case
        status = 'solved'; % Mark as solved
        objective_value = -trace(U_optimal); % Compute the objective value
        return;
    end
